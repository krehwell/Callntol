...
peer.on("connection", (connected) => {
  ...
  $("#info").text(
    "you are data connected to " + caller + " you can chat with him now"
  ); // set the guy who received DOM who is calling

  if (nobodyconnected) {
    // let the user know that the guy who received has been connected
    (async () => {
      try {
        conn = await peer.connect(caller);
        nobodyconnected = false;
      } catch (err) {
        // IIFE with Async
        console.log(err);
      }
  ...
  connected.on("data", (data) => {
    ...
    messagebuffer.prepend(
      `<p id="him"><span class="time">${hour}:${minute}| </span><b>Him:</b>  ${data.message}</p>`
    );
  });
});
...
peer.on("call", async (call) => {
  ...
  Swal.fire({
    ...
    confirmButtonColor: "#179b77",
    denyButtonColor: "#f8333c",
    background: "#fff",
  }).then((result) => {
    console.log("call is", call);
    if (result.isConfirmed) {
      // if pick up then get the media then send and bind to DOM
      getUserMedia({ audio: true, video: true }, (media) => {
      ...
      call.on("stream", (media) => {
        ...
        let mediasource = document.getElementById("videostream");
      ...
      conn.on("close", (media) => {
        ...
        $("#info").html(
          `<b>You are disconnected with the guy you called :[ - Reload to make a call again`
        );
      });
    } else {
      return;
...
peer.on("error", (error) => {
  $("#infoerror").html(
    `Error Occur(<b>${error.type}</b>) - Thinking to reload the browser maybe?`
  );
...
function onErrorMediaCallback() {
  // error handler for not allowed media
  console.log("error on navigator.getUserMedia()");
  $("#infoerror").html(
    `Please allow you browser to use your mic, otherwise how are you gonna communicate asshole`
  );
  setTimeout(() => {
    $("#infoerror").html(``);
  }, 100000); // clear error message in fucking sec
}

// calling the guy
function onClickBtnCall() {
  // user calling the id of the guy
  // call the end user
...
function sendMessage(message) {
  ...
  if (message === "manualtext") {
    message = $("#manualtext").val();
    $("#manualtext").val("");
  }
  if (!message.match(/[^\s]/)) {
    return;
  ...
  messagebuffer.prepend(
    `<p id="me">${data.message}  <b>:Me</b><span class="time">| ${hour}:${minute}</span></p>`
  );
}
